...
def generate_voucher_pdf(customer_info: dict, items: list, payment_info: dict, output_path: str, tracking_number: str = "") -> str:
    ...
    # Title: Trendy K Store
    pdf.cell(0, 10, "Trendy K Store", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    for item in items:
        brand = item.get("category", "").strip()
        name = f"[{brand}] {item['name']}" if brand else item["name"]
        count = item["count"]
        price = item["price"]
        total = item["total"]
        grand_total += total
        ...
        pdf.cell(w_price, height, f"{price:,.0f} Kyat", border=1, align="R")
        pdf.cell(w_total, height, f"{total:,.0f} Kyat", border=1, align="R", new_x="LMARGIN", new_y="NEXT")
        
    # Grand Total row
    pdf.set_font("Pyidaungsu", size=11)
    pdf.cell(w_name + w_count + w_price, 10, "စုစုပေါင်းသင့်ငွေ (Total Amount): ", border=1, align="R")
    pdf.cell(w_total, 10, f"{grand_total:,.0f} Kyat", border=1, align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    if (pay_type == "Full Paid" or pay_type == "Others") and method:
        payment_str += f" ({method})"
        
    # Calculate Paid and Left Amount
    if pay_type == "Full Paid":
        paid_amount_str = f"{grand_total:,.0f} Kyat"
        left_amount_str = "0 Kyat"
    elif pay_type == "Others":
        paid_amount_str = f"{other_amount:,.0f} Kyat"
        left_amount = (grand_total - other_amount) + (del_amount if not del_paid else 0)
        left_amount_str = f"{left_amount:,.0f} Kyat"
    elif pay_type == "COD":
        paid_amount_str = "-"
        ...
        paid_amount_str = "-"
        left_amount_str = f"{grand_total:,.0f} Kyat"
    ...
    pdf.cell(w_total, 10, f"{del_amount:,.0f} Kyat" if del_amount > 0 else "-", border=1, align="R", new_x="LMARGIN", new_y="NEXT")
