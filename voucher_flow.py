...
async def send_voucher_categories(update: Update, context: ContextTypes.DEFAULT_TYPE, message_obj=None) -> int:
    ...
    page = 0
    if update.callback_query and update.callback_query.data.startswith("vcatpage_"):
        try:
    ...
    if categories:
        ...
        for idx, cat in enumerate(current_cats):
            rows.append([InlineKeyboardButton(f"{start_idx + idx + 1}. {cat}", callback_data=f"vcat_{cat}")])
            
        nav_row = []
        if page > 0:
            nav_row.append(InlineKeyboardButton("⬅️ Back", callback_data=f"vcatpage_{page-1}"))
        if page < total_pages - 1:
            nav_row.append(InlineKeyboardButton("Next ➡️", callback_data=f"vcatpage_{page+1}"))
        if nav_row:
            rows.append(nav_row)
    else:
    ...
    text = f"📂 <b>Brand / Category တစ်ခုခုကို ရွေးချယ်ပါရန် (Page {page+1}):</b>"
...
async def handle_voucher_product_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    detail = (
        ...
        f"🔖 Product_Code:       {_escape(product.get('product_code') or 'N/A')}\n"
        f"📐 Size:       {_escape(product['weight'] or 'N/A')}\n"
...
async def handle_payment_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "pay_full" or data == "pay_others":
        context.user_data["voucher"]["payment"]["type"] = "Full Paid" if data == "pay_full" else "Others"
        rows = [
            [InlineKeyboardButton("KPay", callback_data="method_kpay")],
            [InlineKeyboardButton("AYAPay", callback_data="method_ayapay")],
            [InlineKeyboardButton("Wave Money", callback_data="method_wavemoney")],
...
async def handle_full_paid_method(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if method == "Wavemoney": method = "Wave Money"
    
    context.user_data["voucher"]["payment"]["method"] = method
    
    if context.user_data["voucher"]["payment"].get("type") == "Others":
        await query.edit_message_text(
    ...
    try:
        ...
        with open(pdf_path, "rb") as pdf_file:
            await context.bot.send_document(
                chat_id=chat_id,
                document=pdf_file,
                filename=f"Voucher_{safe_name}.pdf",
                caption="📄 Trendy K Store ဘောက်ချာ ဖြစ်ပါသည်။"
            )
    ...
    return ConversationHandler(
        ...
        states={
            ...
            STATE_SELECT_CATEGORY: [
                ...
                CallbackQueryHandler(send_voucher_categories, pattern="^vcatpage_"),
